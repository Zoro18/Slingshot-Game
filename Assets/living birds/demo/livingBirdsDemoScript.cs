...
public class livingBirdsDemoScript : MonoBehaviour {
	...
	void Update () {
        ...
        {
            // Check if the collision is with the boomerang
            if (collision.gameObject.tag == "boomerang")
            {
                // Calculate the force based on the collision's relative velocity
                Vector3 force = collision.relativeVelocity * 500;
                SendMessage("KillBirdWithForce", force);
            }
        }
    }
